...
base_config="/home/d/d_beck26/AdelaiDet_gaze/configs/BoxInst/MS_R_50_1x_VOC2012_gaze.yaml"
no_empty_mask="True"
...
for em in $no_empty_mask; do
  for bl in $baseline; do
    for it in $iters; do
      echo "python" /home/d/d_beck26/AdelaiDet_gaze/tools/train_net.py --config-file  $base_config --num-gpus 4 OUTPUT_DIR /scratch/tmp/d_beck26/AdelaiDet_gaze/training_dir/loss_conf_sweep_2/BoxInst_empty_mask_"$em"_baseline_"$bl"_iters_"$it"/ MODEL.GAZEINST.GAZE_LOSS_NO_EMPTY_MASK $em MODEL.GAZEINST.GAZE_LOSS_LABEL crf MODEL.GAZEINST.GAZE_LOSS_COOLDOWN_BASELINE $bl MODEL.GAZEINST.GAZE_LOSS_COOLDOWN_ITERS $it MODEL.GAZEINST.GAZE_LOSS_LABEL dice
    done;
 done;
done;
